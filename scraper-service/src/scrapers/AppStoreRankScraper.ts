...
export class AppStoreRankScraper implements Scraper {
  ...
  async fetch(): Promise<IndicatorDataPoint | null> {
    console.log(
      `[AppStoreRankScraper] Scraping ${this.appName} from charts page…`,
    );
    ...
    try {
      ...
      const rank = await page.evaluate((searchAppName) => {
        ...
        const lockupElements = Array.from(
          document.querySelectorAll(".we-lockup"),
        );

        for (const lockup of lockupElements) {
          // Get the app title from the lockup
          const titleElement = lockup.querySelector(
            ".we-lockup__title .we-clamp",
          );
      ...
      if (rank === null) {
        ...
        console.log(
          `[AppStoreRankScraper] Found "${this.appDisplayName}" at rank #${rank} AppStore Finance charts`,
        );
        finalRank = rank;
      }

      console.log(
        `[AppStoreRankScraper] Found "${this.appDisplayName}" at rank #${rank} in AppStore Finance charts`,
      );
      ...
      throw toServiceError(
        ...
        `Failed to scrape App Store rank for '${this.appName}'.`,
      );
    } finally {
      if (browser) {
        await browser.close();
