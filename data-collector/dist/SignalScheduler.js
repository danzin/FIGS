...
exports.SignalScheduler = void 0;
...
class SignalScheduler {
    ...
    scheduleSource(sourceKey, config) {
        try {
    ...
    async collectSignal(sourceKey, config) {
        ...
        while (attempt <= config.maxRetries) {
            try {
                ...
                console.error(`[SignalScheduler] Attempt ${attempt}/${config.maxRetries + 1} failed for ${sourceKey}:`, lastError.message);
        ...
        config.consecutiveFailures++;
        const duration = Date.now() - startTime;
        console.error(`[SignalScheduler] Failed to collect ${sourceKey} after ${config.maxRetries + 1} attempts in ${duration}ms. Last error:`, lastError?.message);
        // Disable source if too many consecutive failures
        if (config.consecutiveFailures >= 10) {
            console.warn(`[SignalScheduler] Disabling ${sourceKey} due to ${config.consecutiveFailures} consecutive failures`);
...
exports.SignalScheduler = SignalScheduler;
//# sourceMappingURL=SignalScheduler.js.map