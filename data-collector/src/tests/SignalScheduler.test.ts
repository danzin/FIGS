...
jest.mock("node-cron", () => ({
    schedule: jest.fn().mockImplementation((cronExpression, func, options) => {
        ...
        const mockTask = {
            start: jest.fn(() => {
                // If we want to simulate immediate execution for testing, we can call func() here.
                // For this test, we mostly care that schedule and start are called.
                // To test execution, we might need a more elaborate setup or manual call to func().
            }),
            stop: jest.fn(),
        };
...
const mockMessageBroker: MessageBroker = {
    ...
    consume: jest.fn().mockResolvedValue("consumerTag"), // Added consume
    close: jest.fn().mockResolvedValue(undefined), // Added close
};
...
describe("SignalScheduler Dynamic Registration", () => {
    ...
    test("should schedule and execute a dynamically registered source", async () => {
        ...
        expect(cron.schedule).toHaveBeenCalledWith(dynamicConfig.schedule, expect.any(Function), {"scheduled": false, "timezone": "UTC"});
