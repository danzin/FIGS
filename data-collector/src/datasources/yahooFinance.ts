...
export class YahooFinanceSource implements DataSource {
  ...
  async fetch(): Promise<IndicatorDataPoint[] | null> {
    try {
      ...
      if (!result) {
        console.warn(
          `No chart data returned for ${this.symbol} from Yahoo Finance`,
        );
        return null;
      }
      ...
      if (!meta) {
        console.warn(
          `No meta data available for ${this.symbol} from Yahoo Finance`,
        );
        return null;
      }
      ...
      switch (this.metric) {
        case "price":
          if (
            ...
            console.log(
              `[YahooFinanceSource] Using regularMarketPrice for ${this.symbol}: ${value}`,
            );
          } else {
            ...
            if (timestamps && quotes && quotes.close && timestamps.length > 0) {
              ...
              console.log(
                `[YahooFinanceSource] Using historical close price for ${this.symbol}: ${value}`,
              );
            } else {
        ...
        case "volume":
          if (
            ...
            console.log(
              `[YahooFinanceSource] Using regularMarketVolume for ${this.symbol}: ${value}`,
            );
          } else {
            const timestamps = result.timestamp;
            const quotes = result.indicators?.quote?.[0];
            ...
            if (
              ...
              timestamp = new Date(timestamps[latestIndex] * 1000);
              console.log(
                `[YahooFinanceSource] Using historical volume for ${this.symbol}: ${value}`,
              );
            } else {
        ...
        default:
          throw Errors.validation(`Unknown metric: ${this.metric}`, {
            context: {
              operation: "fetch",
      ...
      if (typeof value !== "number" || isNaN(value) || value === null) {
        console.warn(
          `Invalid ${this.metric} value for ${this.symbol}: ${value}`,
        );
        return null;
      }
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 404) {
          throw Errors.notFound("Yahoo Finance symbol", this.symbol, {
            context: {
              operation: "fetch",
              service: "data-collector",
              sourceKey: this.key,
      ...
      throw toServiceError(
        ...
        `Failed to fetch ${this.metric} for '${this.symbol}' from Yahoo Finance.`,
      );
    }
  }
}
...
class StooqFinanceSource implements DataSource {
  ...
  async fetch(): Promise<IndicatorDataPoint[] | null> {
    try {
      ...
      console.warn(
        `[StooqFinanceSource] Failed to fetch ${this.symbol}:`,
        error,
      );
      return null;
    }
...
export class BrentCrudeOilSource implements DataSource {
  ...
  constructor() {
    ...
    this.sources = BrentCrudeOilSource.BRENT_SYMBOLS.map(
      (symbol) => new YahooFinanceSource(symbol, "price"),
    );
  }
