...
export class DefiLlamaStablecoinSource implements DataSource {
  ...
  async fetch(): Promise<IndicatorDataPoint[] | null> {
    ...
    try {
      ...
      const response = await axios.get<StablecoinHistoryResponse>(
        `${this.baseUrl}/stablecoincharts/all`,
        {
      ...
      const allChainKey = Object.keys(data).find(
        (key) => key.toLowerCase() === "all",
      );
      ...
      const thirtyDaysAgoData = sortedData.find(
        (d) => Math.abs(d.date - thirtyDaysAgoTimestamp) < 86400 * 3,
      );
      ...
      if (thirtyDaysAgoData && yearAgoData) {
        ...
        const sixtyDaysBeforeYearAgoTimestamp = Math.floor(
          sixtyDaysBeforeYearAgo.getTime() / 1000,
        );

        const oldYearAgoData = sortedData.find(
          (d) => Math.abs(d.date - sixtyDaysBeforeYearAgoTimestamp) < 86400 * 7,
        );

        if (oldYearAgoData) {
          const currentYoY =
            ((totalSupply - yearAgoData.totalCirculatingUSD.peggedUSD) /
              yearAgoData.totalCirculatingUSD.peggedUSD) *
            100;
          const pastYoY =
            ((thirtyDaysAgoData.totalCirculatingUSD.peggedUSD -
      ...
      console.log(
        `[DefiLlamaStablecoinSource] Fetched ${results.length} metrics`,
      );
      return results;
    } catch (error) {
      ...
      throw toServiceError(
        ...
        "Failed to fetch stablecoin metrics from DefiLlama.",
      );
    }
  }
}
  ...
  async fetch(): Promise<IndicatorDataPoint[] | null> {
    ...
    try {
      ...
      for (const targetCoin of this.stablecoins) {
        ...
        if (coin) {
          const marketCap =
            coin.circulating?.peggedUSD ||
            coin.circulatingPrevDay?.peggedUSD ||
            0;
      ...
      console.log(
        `[StablecoinBreakdownSource] Fetched ${results.length} stablecoin supplies`,
      );
      return results;
    } catch (error) {
      throw toServiceError(
        ...
        "Failed to fetch stablecoin breakdown from DefiLlama.",
      );
    }
  }
}
