...
export class TimescaleDBService implements DatabaseService {
  ...
  constructor(dbConfig: DbConfig) {
    ...
    this.pool.on("connect", () =>
      console.log("[TimescaleDBService] Connected to database."),
    );
  ...
  public async connect(): Promise<void> {
    try {
      ...
      console.error(
        "[TimescaleDBService] Database connection test failed.",
        error,
      );
      throw Errors.database("Database connection test failed.", {
        cause: error,
        context: {
          operation: "connect",
  ...
  public async insertMarketData(point: MarketDataPoint): Promise<void> {
    ...
    try {
      ...
      const assetName =
        point.asset_symbol.charAt(0).toUpperCase() +
        point.asset_symbol.slice(1).replace(/_/g, " ");
      const assetCategory = "crypto";

      await client.query(assetUpsertQuery, [
        point.asset_symbol,
        assetName,
        assetCategory,
      ]);
      ...
      console.error(
        `[DbService] Transaction failed for market data point. Rolling back.`,
        { point, error },
      );
      throw Errors.database("Failed to insert market data point.", {
        cause: error,
        context: {
          operation: "insertMarketData",
          assetSymbol: point.asset_symbol,
          dataType: point.type,
          service: "signal-persister",
        },
      });
  ...
  public async insertIndicator(point: IndicatorDataPoint): Promise<void> {
    ...
    try {
      ...
      throw Errors.database("Failed to insert indicator.", {
        cause: error,
        context: {
          operation: "insertIndicator",
          indicatorName: point.name,
          service: "signal-persister",
        },
      });
    }
   ...
   */
  public async insertArticleAndSentiment(
    ...
    try {
      ...
      console.log(
        `[DB] Inserting article: external_id=${result.external_id}, published_at=${result.published_at}`,
      );
      ...
      if (articleRes.rows.length > 0) {
        ...
        const selectRes = await client.query(
          "SELECT published_at FROM public.news_articles WHERE id = $1",
          [articleId],
        );
      ...
      console.log(
        `[DB] Article persisted with ID=${articleId}, inserting sentiment`,
      );
      ...
      await client.query("COMMIT");
      console.log(
        `[DB] Successfully committed article and sentiment for: ${result.external_id}`,
      );
    } catch (error) {
      await client.query("ROLLBACK");
      console.error(
        `[DbService] Error in insertArticleAndSentiment transaction:`,
        error,
      );
      throw Errors.database("Failed to insert article and sentiment.", {
        cause: error,
        context: {
          operation: "insertArticleAndSentiment",
  ...
  public async disconnect(): Promise<void> {
    try {
      ...
      throw Errors.database("Failed to close database pool.", {
        cause: error,
        context: {
