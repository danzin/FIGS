...
export class TickerService {
  ...
  async addTicker(request: AddTickerRequest): Promise<AddTickerResponse> {
    ...
    try {
      ...
      console.log(
        `[TickerService] Testing market data for ${request.coinId}...`,
      );
      ...
      console.log(
        `[TickerService] Added market data source: ${marketDataSource.key} with schedule: ${schedule}`,
      );
      ...
      if (request.coinId === "bitcoin") {
        ...
        if (!existingStatus.some((s) => s.sourceKey === dominanceSource.key)) {
          ...
          console.log(
            `[TickerService] Added indicator source: ${dominanceSource.key}`,
          );
        }
      }
      ...
      const appError = logServiceError(
        `[TickerService] Error adding ticker ${request.coinId}:`,
        error,
        {
          operation: "addTicker",
  ...
  removeTicker(sourceKey: string): {
    ...
    try {
      ...
      const appError = logServiceError(
        `[TickerService] Error removing ticker ${sourceKey}:`,
        error,
        {
          operation: "removeTicker",
          service: "data-collector",
          sourceKey,
        },
      );
  ...
  listTickers(): TickerListResponse {
    ...
    const marketDataTickers = status.filter((s) =>
      s.sourceKey.startsWith("coingecko_market_data_"),
    );
    return {
  ...
  async validateCoin(coinId: string): Promise<ValidationResponse> {
    try {
      ...
      const appError = logServiceError(
        `[TickerService] Error validating coin ${coinId}:`,
        error,
        {
          operation: "validateCoin",
          service: "data-collector",
          coinId,
        },
      );
      return {
        valid: false,
        coinId,
        message: `Error validating ${coinId}: ${appError.message}`,
      };
    }
  }
