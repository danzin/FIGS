...
export class BitcoinHistoricalSource implements DataSource {
	...
	async fetch(): Promise<MarketDataPoint[] | null> {
		...
		try {
			...
			console.error("[BitcoinHistoricalSource] Error:", error);
...
export class PowerLawIndicatorSource implements DataSource {
	...
	async fetch(): Promise<{ name: string; time: Date; value: number; source: string }[] | null> {
		const now = new Date();
		const results: { name: string; time: Date; value: number; source: string }[] = [];
		...
		try {
			...
			const daysSinceGenesis = Math.floor((now.getTime() - this.genesisDate.getTime()) / (1000 * 60 * 60 * 24));

			// Calculate Power Law fair value
			// Formula: log(Price) = intercept + slope * log(days)
			const fairValue = Math.pow(10, this.intercept + this.slope * Math.log10(daysSinceGenesis));
			...
			if (prices && prices.length >= 1400) {
				...
				const stdDev = Math.sqrt(squaredDiffs.reduce((a, b) => a + b, 0) / priceValues.length);
			...
			console.log(`[PowerLawIndicatorSource] Calculated ${results.length} indicators`);
			return results;
		} catch (error) {
			console.error("[PowerLawIndicatorSource] Error:", error);
			if (axios.isAxiosError(error) && error.response?.status === 429) {
