...
# RUN npm run build -w data-collector

# Prune devDependencies to clean up node_modules for production.
...
COPY --from=builder --chown=node:node /usr/src/monorepo/data-collector/dist ./dist

# 4. Copy this service's package.json for runtime context
COPY --from=builder --chown=node:node /usr/src/monorepo/data-collector/package.json ./
