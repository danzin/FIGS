...
export class HealthService {
	...
	private checkSchedulerHealth(): ComponentHealth {
		try {
			...
			let componentStatus: "healthy" | "degraded" | "unhealthy" = "healthy";
	...
	private async checkMessageBrokerHealth(): Promise<ComponentHealth> {
		try {
	...
	private checkDataSourcesHealth(): ComponentHealth {
		try {
	...
	private checkMemoryHealth(): ComponentHealth {
		try {
			...
			const HEAP_CRITICAL_THRESHOLD_MB = config.HEALTH_HEAP_CRITICAL_MB || 768;
			const HEAP_WARNING_THRESHOLD_MB = config.HEALTH_HEAP_WARNING_MB || 512;
