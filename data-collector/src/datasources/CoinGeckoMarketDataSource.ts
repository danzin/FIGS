...
export class CoinGeckoMarketDataSource implements DataSource {
  ...
  async fetch(): Promise<MarketDataPoint[] | null> {
    try {
      ...
      if (!coinData) {
        console.warn(
          `[CoinGeckoMarketDataSource] No data returned for ID '${this.coinGeckoId}' from CoinGecko`,
        );
        return null;
      }
      ...
      if (typeof priceValue === "number" && !isNaN(priceValue)) {
        ...
        console.warn(
          `[CoinGeckoMarketDataSource] Invalid price value for '${this.coinGeckoId}': ${priceValue}`,
        );
      }
      ...
      if (typeof volumeValue === "number" && !isNaN(volumeValue)) {
        ...
        console.warn(
          `[CoinGeckoMarketDataSource] Invalid volume value for '${this.coinGeckoId}': ${volumeValue}`,
        );
      }
      ...
      throw toServiceError(
        ...
        `Failed to fetch market data for '${this.coinGeckoId}' from CoinGecko.`,
      );
    }
  }
}
