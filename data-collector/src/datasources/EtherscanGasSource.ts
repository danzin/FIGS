  ...
  async fetch(): Promise<IndicatorDataPoint[] | null> {
    try {
      ...
      if (response.data?.status !== "1" || !response.data?.result) {
        console.warn(
          `[EtherscanGasSource] Invalid response:`,
      ...
      throw toServiceError(
        ...
        "Failed to fetch Ethereum gas prices.",
      );
    }
  }
}
...
export class OwlracleGasSource implements DataSource {
  ...
  async fetch(): Promise<IndicatorDataPoint[] | null> {
    try {
      ...
      const dataPoints: IndicatorDataPoint[] = speeds.map(
        (speed: { acceptance: number; gasPrice: number }) => ({
      ...
      throw toServiceError(
        ...
        `Failed to fetch ${this.chain} gas prices from Owlracle.`,
      );
    }
  }
}
